...
export function editContatoModel(idDoContatoParaAtualizar, novosDados){
    const indiceDoContato = contatos.findIndex((contato) => contato.id == idDoContatoParaAtualizar);
    if (indiceDoContato >= 0) contatos[indiceDoContato] = { ...contatos[indiceDoContato], ...novosDados };
    return contatos;
};

export function deleteContatoModel(idDoContato){
    const listaDeContatosAtualizada = contatos.splice(idDoContato-1, 1);
    return listaDeContatosAtualizada;
};